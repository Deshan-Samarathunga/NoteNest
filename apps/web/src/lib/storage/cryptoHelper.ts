...
import { encryptionEnabled } from './env';
...
export function maybeEncrypt(payload: unknown, passphrase?: string) {
  if (!encryptionEnabled()) return payload;
  if (!passphrase) throw new Error('passphrase_required');
  return encryptJson(payload, passphrase);
}

export function maybeDecrypt<T>(payload: unknown, passphrase?: string): T {
  if (!encryptionEnabled()) return payload as T;
  if (!passphrase) throw new Error('passphrase_required');
